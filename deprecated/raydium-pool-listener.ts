...
function subscribeToNewRaydiumPools(): void {
    ...
    connection.onProgramAccountChange(new PublicKey(RAYDIUM_POOL_V4_PROGRAM_ID),  async (updatedAccountInfo) => {
        const poolId = updatedAccountInfo.accountId.toString();
        if (seenTransactions.includes(poolId)) {
          return
        }
        seenTransactions.push(poolId);
        const deserialize1 = spltoken.AccountLayout.decode(updatedAccountInfo.accountInfo.data);
        const deserialize2 = LIQUIDITY_STATE_LAYOUT_V4.decode(updatedAccountInfo.accountInfo.data);
        const deserialize3 = MARKET_STATE_LAYOUT_V3.decode(updatedAccountInfo.accountInfo.data);
        console.log('====================================================================');
